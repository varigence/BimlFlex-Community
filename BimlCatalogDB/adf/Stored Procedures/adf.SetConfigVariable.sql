...
*/
CREATE PROCEDURE [adf].[SetConfigVariable]
...
BEGIN TRY
	...
	IF NOT EXISTS (SELECT 1 FROM [adf].[ConfigVariable] WHERE [SystemName] = @SystemName AND [ObjectName] = @ObjectName AND [VariableName] = @VariableName)
	BEGIN
	...
	BEGIN
		...
		IF (UPPER(ISNULL(@VariableValue, '')) IN ('', 'NULL', '0') OR UPPER(ISNULL(LEFT(@VariableValue, 10), '')) IN ('1753-01-01', '1899-12-31', '1900-01-01', '0000-01-01'))
			AND (UPPER(ISNULL(@PreviousValue, '')) NOT IN ('', 'NULL', '0') OR UPPER(ISNULL(LEFT(@PreviousValue, 10), '')) NOT IN ('1753-01-01', '1899-12-31', '1900-01-01', '0000-01-01'))
			SET @VariableValue = @PreviousValue; 

		IF (@CurrentValue <> COALESCE(@VariableValue, @CurrentValue, ''))
		BEGIN
			BEGIN
				UPDATE	[adf].[ConfigVariable]
