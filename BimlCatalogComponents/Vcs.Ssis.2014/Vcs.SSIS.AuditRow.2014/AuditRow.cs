...
namespace Vcs.SSIS
{
    [
        DtsPipelineComponent
            (
            DisplayName = "VCS Row Audit"
            , Description = "Logs data audit rows as XML via the Vcs Log Provider. Can log distinct rows"
            , ComponentType = 0
            , IconResource = "Vcs.SSIS.AuditRow.ico"
            )]
    public class AuditRow : PipelineComponent
    {
        ...
        public override void PostExecute()
        {
            ...
            bool isTrue;
            ComponentMetaData.FireError(0, ComponentMetaData.Name, "Vcs Row Audit: Rows were logged and RowLogType = HardError.", "", 0, out isTrue);
        }
        ...
        public override void ProcessInput(int inputID, PipelineBuffer buffer)
        {
            ...
            while (buffer.NextRow())
            {
                ...
                else
                {
                    foreach (var info in _inputColumnInfos)
                    {
                        ...
                        // Remove this for now until we fix ErrorDescription Component
                        //switch (info.Name)
                        //{
                        //    case "ErrorCode":
            ...
            bool isTrue;
            ComponentMetaData.FireError(0, ComponentMetaData.Name, "Vcs Row Audit: Too many rows logged, LimitNumberOfRowsToLog is negative and exceeded.", "", 0, out isTrue);
        }
